...
def run(engine, *, ds=None, ms=None, pools=None, end=THIRD, **kwargs):
    if ds is None:
        ds, ms = frames()
...
def test_contract_t_fill_then_entry_day_eod_then_next_open_no_reentry(engine):
    # D=Friday, T=Monday: signal pool already shifted strictly after D by exporter.
    entry = run(engine, end=T)
    ...
    state = run(engine, pools={T: [CODE], NEXT: [CODE]})
...
@pytest.mark.parametrize("engine", [daily, minute])
def test_green_entry_above_sma_holds_through_red_then_breaks(engine):
    ds, ms = frames(closes=(10, 10.5, 10.4, 9.8))
    state = run(engine, ds=ds, ms=ms, end=NEXT)
    assert len(fills(state)) == 1
    assert state.positions[CODE][0].pending_exit == ""  # red vs T but above SMA5
    state = run(engine, ds=ds, ms=ms)
    assert len(fills(state)) == 1  # T+2 EOD can only schedule T+3
    assert state.positions[CODE][0].pending_exit == "ma_signal:SMA5"


@pytest.mark.parametrize("engine", [daily, minute])
def test_green_entry_below_sma5_schedules_exit_on_entry_day(engine):
    ds, ms = frames(closes=(9, 9.5, 10, 10), opens=(9, 9.4, 9.6, 10))
    state = run(engine, ds=ds, ms=ms, end=T)
    assert len(fills(state)) == 1
    assert state.positions[CODE][0].pending_exit == "ma_signal:SMA5"
    extended = run(engine, ds=ds, ms=ms, end=NEXT)
    assert fills(extended)[1]["date"] == NEXT


@pytest.mark.parametrize("engine", [daily, minute])
def test_limit_up_skip_consumes_signal_and_actual_chase_queue_stays_empty(engine, monkeypatch):
...
@pytest.mark.parametrize("engine", [daily, minute])
def test_pending_down_limit_defers_to_next_day_open_despite_intraday_rebound(engine):
...
def test_open_buy_skip_does_not_retry_later_minute_or_day(mode):
    ds, ms = frames()
...
def test_pending_sell_waits_for_next_open_when_no_open_or_zero_volume(mode):
    ds, ms = frames()
...
def test_volume_a_unfinished_open_bucket_buy_skips_before_lookup():
    calls = []
    ...
    state = run(minute, participation_rate=.1, volume_for_bucket=lookup)
...
def test_volume_a_unfinished_open_bucket_pending_sell_defers_preserves_position(monkeypatch):
    ...
    state = run(minute, pools={}, participation_rate=.1, volume_for_bucket=unavailable_lookup)
...
def test_cap_off_never_looks_up_volume_and_keeps_open_fills():
    def forbidden(*_):
        pytest.fail("cap-off does not consult capacity")

    state = run(minute, volume_for_bucket=forbidden)
