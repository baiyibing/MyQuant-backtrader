...
# ---------------------------------------------------------------------------
# 策略6（version6，2026-09-10）：4% 盘中止损 + 2% 锚定分档回撤止盈
# ---------------------------------------------------------------------------
# 策略6（version6，2026-09-10）：4% 盘中止损 + 2% 锚定分档回撤止盈
...
def test_v6_factory_registered_with_expected_defaults():
    ...
    assert s.params["stop_loss_pct"] == 0.04
    assert s.params["profit_base_pct"] == 0.02
...
def test_v6_stop_loss_triggers_at_4pct_not_3pct():
    s = _v6()  # 峰值=成本（无正利润历史），隔离止损分支
    ok, reason = s.should_sell(_st(cost=10.0, high=10.0), _dt(2025, 11, 3, 10, 0), 9.611)
    assert not ok  # -3.89% 未触发
    ok, reason = s.should_sell(_st(cost=10.0, high=10.0), _dt(2025, 11, 3, 10, 1), 9.598)
    assert ok and reason.startswith("stop_loss")


def test_v6_t1_trailing_50pct_of_peak_excess_above_2pct():
    s = _v6()
    now = _dt(2025, 11, 3, 10, 0)
    # 峰值 10.5（+5%，超额+3%）；T+1 档 50% → 触发线 = 1.02 + 0.5*0.03 = +3.5% → 10.35
    ok, reason = s.should_sell(_st(cost=10.0, high=10.5, days=1), now, 10.352)
    assert not ok
    ok, reason = s.should_sell(_st(cost=10.0, high=10.5, days=1), now, 10.348)
...
def test_v6_t2_uses_40pct_t3plus_uses_30pct():
    ...
    # 峰值 +5%（超额 3%）：T+2 触发线 = 1.02 + 0.4*0.03 = +3.2% → 10.32
    ok, _ = s.should_sell(_st(cost=10.0, high=10.5, days=2), now, 10.322)
    assert not ok
    ok, _ = s.should_sell(_st(cost=10.0, high=10.5, days=2), now, 10.318)
    assert ok
    # T+3+ 档 30%：触发线 = 1.02 + 0.3*0.03 = +2.9% → 10.29
    ok, reason = s.should_sell(_st(cost=10.0, high=10.5, days=5), now, 10.288)
    assert ok and "T+5" in reason
    ok, _ = s.should_sell(_st(cost=10.0, high=10.5, days=5), now, 10.292)
    assert not ok


def test_v6_fall_below_2pct_anchor_after_peak_still_trails():
    s = _v6()
    # 峰值曾 +5%，现价跌破 +2% 锚 → 比值 < 档位，仍触发（保护性离场）
    ok, reason = s.should_sell(_st(cost=10.0, high=10.5, days=1), _dt(2025, 11, 3, 14, 0), 10.10)
    assert ok and reason.startswith("profit_take:drawdown")


def test_v6_positive_profit_trail_when_peak_below_2pct():
    s = _v6()
    now = _dt(2025, 11, 3, 10, 0)
    # 峰值 +1%（未达 +2% 锚）：正利润 50% 回撤 → 触发线 +0.5% → 10.05
    ok, reason = s.should_sell(_st(cost=10.0, high=10.10, days=1), now, 10.052)
    assert not ok
    ok, reason = s.should_sell(_st(cost=10.0, high=10.10, days=1), now, 10.048)
    assert ok and "正利润" in reason


def test_v6_below_cost_without_stop_or_positive_peak_observes():
    s = _v6()
    # 开盘价 < 买入价、无正利润峰值、未触发 4% 止损 → 观察（无动作）
    ok, _ = s.should_sell(_st(cost=10.0, high=10.0, days=1), _dt(2025, 11, 3, 9, 31), 9.80)
    assert not ok
    ok, _ = s.should_sell(_st(cost=10.0, high=10.0, days=1), _dt(2025, 11, 3, 9, 32), 9.95)
    assert not ok
...
def test_v6_adapter_routes_sell_to_local_strategy():
    ...
    ok, reason = adapter.should_sell(_st(cost=10.0, high=10.0), _dt(2025, 11, 3, 10, 0), 9.598)
    assert ok and reason.startswith("stop_loss")
