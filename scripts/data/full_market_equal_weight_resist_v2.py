...
用法：
  python scripts/data/full_market_equal_weight_resist_v2.py --date 20260515
  python scripts/data/full_market_equal_weight_resist_v2.py --date 20260515 --window 120
"""
...
def _calc_one(args) -> Optional[Dict[str, Any]]:
    ...
    # ── canonical 路径 ──
    try:
        arr_t = adapt_columns(cast(Any, win_t.copy()), stock_code=code, as_of_date=as_of_t)
        arr_y = adapt_columns(cast(Any, win_y.copy()), stock_code=code, as_of_date=as_of_y)
    except Exception:
        return None
    ...
    bb = bb_position(cast(Any, win_t["close"].values))
