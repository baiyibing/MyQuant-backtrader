...
def _process_stock_original(code: str, df: pd.DataFrame, target: pd.Timestamp, window: int, step: float,
    ...
    cyqk_circ_t = _canonical_cyqk(df_t, code, window=window, step=step, use_free_float=False,
                                   capital_override=circ_cap_t_val)
    cyqk_circ_prev = _canonical_cyqk(df_prev, code, window=window, step=step, use_free_float=False,
    ...
    if free_missing:
        ...
        cyqk_free_t = _canonical_cyqk(df_t, code, window=window, step=step, use_free_float=True,
                                       capital_override=free_cap_t_val)
        cyqk_free_prev = _canonical_cyqk(df_prev, code, window=window, step=step, use_free_float=True,
...
def run(date_str, output, window=1000, step=0.01,
    ...
    for _, row in cap_df.iterrows():
        if row['which'] == 'T':
            cap_t[row['stock_code']] = row['circulating_capital'] if row['circulating_capital'] and row['circulating_capital'] > 0 else 0
            free_t[row['stock_code']] = row['freeFloatCapital'] if row['freeFloatCapital'] and row['freeFloatCapital'] > 0 else 0
        else:
            cap_tp[row['stock_code']] = row['circulating_capital'] if row['circulating_capital'] and row['circulating_capital'] > 0 else 0
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for code in codes:
            ...
            if df is None:
                skipped += 1
                continue
            futures[executor.submit(process_fn, code, df, target, window, step,
                                     cap_t, cap_tp, free_t, free_tp,
