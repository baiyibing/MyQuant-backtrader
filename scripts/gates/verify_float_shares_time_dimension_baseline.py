...
from oskh_factors.chip.core import turnover_chip_factors
from oskh_factors.chip.shares import _get_float_shares
...
FLOAT_SHARES_HISTORY_PATH = str(
    resolve_parquet_container() / "float_shares_history.parquet"
)
...
def _turnover_and_factor_baseline(
    ...
    for code in codes:
        ...
        vol = np.asarray(df["volume"].astype(np.float64).values, dtype=np.float64)
        dt = df["dt"]
        static_fs = _get_float_shares(code)
        tr_static = np.asarray(vol * 100.0 / static_fs, dtype=np.float64)

        fs_time = np.array([_get_float_shares(code, d) for d in dt], dtype=np.float64)
        tr_time = np.asarray(vol * 100.0 / fs_time, dtype=np.float64)
        ...
        if len(df) >= window:
            ...
            try:
                f_static = turnover_chip_factors(cast(Any, tr_static), cast(Any, close), window=window)
                f_time = turnover_chip_factors(cast(Any, tr_time), cast(Any, close), window=window)
...
def main() -> None:
    ...
    metrics: Dict[str, Any] = {"sample_size": len(codes), "window": int(args.window), "bars": int(args.bars)}
