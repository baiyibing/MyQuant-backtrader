...
用法：
    python backtest/verify_mvp_min.py              # 随机抽样 15 只验证
    python backtest/verify_mvp_min.py --all          # 全部标的（耗时较长）
    python backtest/verify_mvp_min.py --stocks 000001.SZ,000002.SZ
...
def code_to_df(stock_code: str, reader=None) -> pd.DataFrame | None:
...
def verify_single(stock_code: str, reader=None) -> dict:
    ...
    # 1) _adapt_columns
    try:
        arr = adapt_columns(cast(Any, df), stock_code=stock_code)
...
def collect_stocks(limit: int | None = None) -> list:
