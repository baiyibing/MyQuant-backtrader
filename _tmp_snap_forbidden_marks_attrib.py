...
forbidden = []
# prior out leaves under phase (#181..#187 + older)
for name in [
    ...
    if p.exists():
        forbidden.append(p)
# phase timings / receipts / pstats (prior knives only; marks_attrib run writes new names)
for pat in [
...
# union with prior capacity snapshot path list (guard must be superset)
prior_snap = phase / "_tmp_forbidden_mtime_snapshot_capacity_sparse.json"
if prior_snap.exists():
    import json as _json
    prior = _json.loads(prior_snap.read_text(encoding="utf-8"))
...
out = phase / "_tmp_forbidden_mtime_snapshot_marks_attrib.json"
