...
| Path | Role |
|------|------|
| `backtest/research/csv_daily_backtest.py` | 向量化日线（策略 1/2/5/6/8 策略书） |
| `backtest/research/csv_minute_backtest.py` | 向量化分钟（策略 1/2/5/6/8） |
| `backtest/research/csv_minute_backtest_v7.py` | 策略 7 金榕元仓位机（独立） |
| `backtest/research/csv_pool.py` | 名单 CSV：裸六位码 → canonical |
| `backtest/research/csv_strategy_books.py` | 1/2/5/6/8 策略书 |
...
必须 `--strategy version1|version2|version5|version6|version8`。7 必须 `--pool-dir`。数据经 `oskh_data` / `resolve_period_root`（有 `F:\stock_data\.authority` 时跟 F 盘）。
