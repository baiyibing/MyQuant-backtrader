...
> **Status**: **v0.1 · docs-only plan / 待人裁**。本 PR 仅提交本计划；未实施 Slice B 测试或 Slice C 验收，不代表 A→B→C 已交付。
> **Main ship / 单行范围**: 仅 #112 P3 **δ2**：细化已落地 E-R6 的除权事件、lot 参考价重标定与经济残留契约，规划未来 data-free pins；默认 **零生产行为变更**。
> **IMPLEMENTATION_BASE（本 worktree 分支起点，已用 `git rev-parse HEAD` 核对）**: `1ad010cca013afe8186f20275cbdcca71e500823`。
...
| 路径 | As-built 边界 | 锚点 |
|---|---|---|
| daily `run()` | `use_qlib_bins` 或 `dividend_type != none` 时 `exdiv=None`；front/back/qlib 连续域跳过 E-R6；lake none 才加载 map | `backtest/research/csv_daily_backtest.py:583-588`、`:618` |
...
P3δ2.5 的 A 仅保持上表：保留 daily 已有的连续域跳过，不把它补接到 minute/v7，不声称后两者已关闭混域风险。`--qlib-cost` 是 δ1 费率选项，不能用它判定价格域或代替 `use_qlib_bins`。
...
|---|---|---|
| 触发参考错域 | 已对可处理事件缩放存量参考价，并把 prev_close 映射到 D 域档位；既有假止损/档位 vectors 已存在 | 钉清事件门、k 方向、字段集合、调用顺序及可处理前提，不重写算法 |
...
| `tests/test_ashare_session.py:21` | session 昨收映射；board/ST 档位 | mapped prev_close 到 Decimal 档位的完整数值链，保留 None 行为 |
| `tests/test_csv_minute_backtest_v7.py:238`、`:295` | 官方昨收映射与 trial stop 的公开 `simulate_v7` 路径 | 全字段缩放清单、多 lot、新买/加仓不重乘、现金与股数不动 |
...
| ID | 硬锁 |
|---|---|
| **F-R1** | 本 PR **只写本 plan**；禁止生产 Python、测试、配置/依赖/CI 修改。未来人裁 A 才允许 §7 列出的 docs + data-free tests；生产仍零 diff。 |
...
这里 **A = 保持 as-built，本刀只契约化 + 未来 data-free pins**。B/C 仅供后续选择，不包含在本 PR 或默认实施授权内；任一 B/C 都须先更新范围、生产冻结与验收方案，不能顺带实施。旧 P1–P4 与 δ1 P3.1/2/3 不重裁。
...
| **P3δ2.5** | front/back/qlib 连续域跳过 E-R6 是否保持 | 保留 daily `run()` 现有跳过；minute/v7 现存差异照录并冻结 | 另案统一所有入口的价格域/跳过接线 | 另案引入显式域类型与拒绝混域校验 |

---

## 6) Non-goals

- 不重开 E-R6 参考价算法或旧宿主 Slice D，不承诺纠正全部假止损、停牌遗漏或经济亏损。
- 不增股、不发放现金红利、不补历史成交、不重算总回报、不统一 v7/book 的 lot 模型。
- 不修改噪声阈值、因子构建、缺失政策、warmup 长度、v4 SMA 价格域或 loader；不采集公司行动数据。
...
前置：§5 后续人裁录入。本 PR 只提供 v0.1 可审计划；下面每片均为未来动作。
...
|---|---|---|
| B1 事件门与 k | `tests/test_exdiv_map.py` | §2.2 四格、两个阈值等号/两侧、0.8% 有/无事件、k<1/k>1、有效行 LAG/warmup、缺 ex fallback；合成 parquet 显式 tmp_path，绝不走真实 resolver |
| B2 缩放范围与顺序 | `tests/test_exdiv_refprice_engines.py` | 多 lot 的 cost/peak ×k，其余字段保留；公开 daily/minute `simulate` 证明老 lot 仅一次、新买不重乘、连续事件复合；保留 helper 非幂等事实；缺 bar 前置分支不伪装成递延补偿 |
| B3 v7 保留分叉 | `tests/test_csv_minute_backtest_v7.py` | entry_A/avg_cost/peak/add1_A1/Lot.price 完整集合及不变量；公开 `simulate_v7` 验证扫描前一次性、新买/加仓、原股数/现金；不只调用私有 helper |
| B4 昨收到档位 | `tests/test_ashare_session.py`、`tests/test_exdiv_refprice_engines.py` | `raw_prev=10,k=0.5→5→(5.50,4.50)` 普通 10% 档；复用 held/chase/pool 既有 vectors，保持 Decimal 档位与 `limits=None` 现状 |
| B5 经济残留 | `tests/test_exdiv_refprice_engines.py`、`tests/test_csv_minute_backtest_v7.py` | §2.4 数值 oracle；参考字段变、股数现金不因事件变，raw mark 仍有跳变；不把 EOD_MARK 当真实 SELL 或给 v7 加产物列 |
| B6 价格域入口 | `tests/test_exdiv_refprice_engines.py` | 对 daily `run()` 的 lake none/front/back/qlib 四态隔离所有 I/O：stub bars/pool、map loader、simulate、artifact writer；捕获 exdiv 参数与 loader 调用。minute/v7 无跳过的现状只作受控接线证据，不改生产补齐 |
...
路径通过条件：本 PR 仅本 plan；未来 A/B/C 仅本 plan、`docs/backtest/engine-ashare-correctness.md`、`docs/backtest/README.md` 与 B1–B6 四个具名测试文件。§8.2 费率/fence 回归只跑不改。任何其它路径变化先解释来源，不计作本刀通过；严禁用“冻结表未列出”授权修改其它生产文件。未来每个变更 `.md` 也要执行 UTF-8/BOM/NUL 检查，`.py` 同要求。
