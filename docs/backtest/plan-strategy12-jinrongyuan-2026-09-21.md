...
- P10 follow-up（#151 option 2）采用混合域：version12 日线信号固定 `1d/front`；分钟成交默认 `1m/none`，`--dividend-type front` 仅作可选 fail-closed（缺 `1m/front` 立即失败）；front 日线 + none 分钟下禁用隐式 exdiv map，仅允许显式 economics 路径，避免静默双重调整。MA10 预热使用 22 个日历日 slack。
- Slice D（日线）文档收口仅记路径（不含 NAV/returns/parity 结论）：`docs/backtest/reviews/slice-d-daily-front-smoke-newtest_4090-2026-09-21.md`。
