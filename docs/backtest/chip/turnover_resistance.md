...
- `turnover-resist/` — Rust 高性能实现（`maturin build --profile release-fast` 构建 PyO3 wheel；`cargo build --release` 构建 CLI binary）。PyO3 FFI 桥接见 [`oskh_core/turnover_resist_bridge.py`](../../oskh_core/turnover_resist_bridge.py)（自动检测 `pyo3_ffi` / `cli_fallback`，一行 `compute_turnover_resist(date=...)` 调用）。桥接选型见 [`turnover-resist-bridge-selection.md`](turnover-resist-bridge-selection.md)（CLI 39ms vs PyO3 0.2μs，结论 PyO3）。
...
| 文件 | 作用 |
|------|------|
| `backtest/chip_algorithm.py` | 适配层：`adapt_columns`、`derived_chip_factors`、`compute_crossday_turnover_resistance`、`compute_chip_factors`、`compute_equal_weight_cyqk`（市面指标对比） |
...
| `backtest/daily_chip_logger.py` | **Legacy TR_80 每日截面 CSV**：`derived_chip_factors` + ARC/VRC |
| `backtest/filter_chip_stocks.py` | 指定日截面：阻力 + BB 过滤（列名 `resist`） |
| `oskh_core/turnover_resist_bridge.py` | **PyO3 FFI 桥接**（v1.0, 2026-06-07）：`compute_turnover_resist()` 主入口，自动选择 `pyo3_ffi` 或 `cli_fallback` |
...
| `scripts/full_market_chip_resist.py` | 全市场 CSV（`turnover_resist` 等） |
| `scripts/spot_check_chip_factors.py` | 随机抽样 Spot Check |
| `scripts/verify_turnover_resistance_alignment.py` | canonical vs 脚本路径换手率/阻力偏差验证 |
...
| `backtest/chip_turnover_resistance_bands.py` | TR 布林带 rolling 计算 |
| `scripts/compute_turnover_resistance_bands.py` | 盘后 TR + TR BB 单日流水线 |
| `scripts/backfill_turnover_resistance_bands.py` | 历史 TR 回填 + TR BB |
...
| 双路径验证 | ✅ 通过 | `scripts/verify_rust_python_alignment.py --date 20260525`：4595 只全 diff=0 |

验证命令：
```bash
D:/anaconda3/envs/vanna311/python.exe scripts/verify_rust_python_alignment.py \
