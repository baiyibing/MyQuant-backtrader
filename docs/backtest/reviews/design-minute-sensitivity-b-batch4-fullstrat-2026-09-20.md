...
| `clock_next_open_fullstrat` | `next_tradable_open_research` | 0 | `FILLABLE`，待实现及 pins |
...
H2 全部成交交换、严格同日到期、UNFILLED / 全现金合法、卖单到期清除并次日策略重评继续绑定。日内现金、持仓与阶段只在实际成交时更新，未来卖出不得资助更早买单。新语义分叉仍 PR comment + stop；Q2 已有明确人裁，无需重问。
