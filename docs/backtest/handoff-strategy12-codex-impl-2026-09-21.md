...
4. 减仓基数 = **t1_sellable** lots 合计 ×50%，floor 100 股；**同一「下方周期」只减一次（latch）**（P11），收复清零后再武装。
...
**测试**：`tests/test_strategy12_rules.py` data-free——MA 不足 5/10 根→None、latch 语义、50% 取整、lot0 保底、顺序。
