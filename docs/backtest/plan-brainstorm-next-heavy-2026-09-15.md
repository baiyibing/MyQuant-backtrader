...
- 日期：2026-09-15
- 状态：队列草案（H1–H13 软卫生 + **H14/D1 profile ✓** 后的 **重活** 顺序）
...
软队列 H1–H13 已覆盖 docs/CLI/L2 fence/CI gates/list-quality/chip inventory/CYQ–TR 边界；**D1 profile 已完成（H14）**。下列为继续 brainstorm / 开片的有序重活；**不是**已批准的实现承诺。
...
| 2 | **D2** | D（chip offload） | 按 D1：**numba/向量化 `minute_chip_distribution`**（语义锁 + 对拍）；hybrid/curpdf/cumpdf **leave** | 非 full Rust rewrite；非 MyQuant CYQ feeder；不改卖点 |
| 3 | **C soft+** | C（list-quality） | 在 H9 之上加深 list-quality（更多直方图 / overlap / 目录健康；仍吃契约 CSV） | **非** run-manifest hard；不接 MyQuant manifest 消费 |
...
- `minute_chip_distribution` ~**50.7 ms**/call（80×240）；cProfile：Python `for` + 每 bar `np.zeros` → **D2 = numba 优先**
...
- Inventory §D 其它 later offload（随 D2 实施再刷 recommendation）
