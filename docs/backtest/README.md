...
**策略 8.4（stop10-tp10-reserve，止盈改为 20%）**：`--strategy version8_4`（别名 `8.4` / `v8_4`）。止损 10%，T+1 起现价达到买价 +20% 止盈，涨停保留至开板，名单全加，独立 +20% 台阶，上证十日线两日下方停开新仓、已持可加。不改现行 `version8`。止盈 +10% 的原结果留在 `csv_minute_v8_20251023_20260909_v8_stop10_tp10_reserve/` 与 `csv_minute_v8_4_20251023_20260909/`。

**策略 8.5（8.4 止盈 4% / 峰差 30 / T+4 收盘清）**：`--strategy version8_5`（别名 `8.5` / `v8_5`）。止损仍是 10%。T+1 起现价达到买价 +4% 止盈。同会话创新高后 30 分钟内不评这笔止盈。持仓最高价一直低于买价 ×1.04 的，T+4 收盘清（分钟用 15:00 那根；没有则用当天最后一根）。涨停保留、名单全加、+20% 台阶、上证闸沿用 8.4。计划：[plan-v8-5-tp4-gap30-t4close-2026-09-23.md](plan-v8-5-tp4-gap30-t4close-2026-09-23.md)。

**策略 8.6（止损 2% / 保底 ×1.004 留 80% 涨幅 / 峰差 15 分钟 / 跌破线即卖）**：`--strategy version8_6`（别名 `8.6` / `v8_6`）。T+1 起峰值到过买价 ×1.004 后，离场线 = max(买价×1.004, 买价+最高涨幅×80%)，现价回到该线即卖，跌破买价也卖。峰差 15 分钟。没到 ×1.004 的仓 T+1 收盘清。止损 2%。名单再现仍加 100 万。对照：止损 10% 的 armexit 在 `csv_minute_v8_6_20251023_20260909_stop10_floor1004_gap15_armexit/`。计划：[plan-v8-6-stop6-t2close-nostep-2026-09-23.md](plan-v8-6-stop6-t2close-nostep-2026-09-23.md)。
...
| **CSV 盘后人工分析包（固定文件名；不重跑）** | [prompt-csv-human-analysis.md](prompt-csv-human-analysis.md) · `scripts/research/export_csv_human_analysis.py --run-dir <run>` |
