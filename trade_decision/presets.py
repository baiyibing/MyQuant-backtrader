...
from common.infra.timekeeping import to_shanghai
...
SellPresetEvaluator = Callable[[Mapping[str, Any], "SellPresetContext"], "SellPresetDecision"]
SellPresetBuyGate = Callable[[Mapping[str, Any], float, Optional[Mapping[str, Any]]], bool]
...
def register_sell_preset(
    ...
    *,
    ...
    with _PRESET_REGISTRY_LOCK:
        ...
        _SELL_PRESET_BUY_GATES[key] = buy_gate or (lambda _params, _price, _ind: True)
...
_PRESETS_WITHOUT_COST_PRICE = frozenset(
    {"version4", "rebalance_driven", "rotation_driven", "signal_exit"}
)


def _preset_requires_cost_price(preset: str) -> bool:
    """MA-only and signal-driven presets skip NULL-cost wrapper."""
    return canonical_preset_name(preset, fallback="version1") not in _PRESETS_WITHOUT_COST_PRICE
...
def evaluate_sell_preset(
    ...
    if _preset_requires_cost_price(preset) and (
        ...
        get_logger("SellRules", "null_cost_skip", trace_id=get_trace_context()).warning(
            "avg_cost_yuan is None; preset sell SKIPPED",
            context={"reason": "data_corruption_null_cost", "preset": str(preset or "")},
        )
...
def allow_buy_by_preset(
    ...
    *,
    ...
    with _PRESET_REGISTRY_LOCK:
        p = _effective_preset_or_raise(preset)
        gate = _SELL_PRESET_BUY_GATES.get(p) or (lambda _params, _price, _ind: True)
    return bool(gate(params, float(current_price), indicators))
...
def _allow_buy_v4(
    ...
    if ma_val <= 0:
        return False
    return float(current_price) >= ma_val
...
def _eval_v4(params: Mapping[str, Any], ctx: SellPresetContext) -> SellPresetDecision:
    ...
    if ma_val <= 0 or ctx.current_price <= 0:
        _log.warning(
...
def _eval_rebalance_driven(params: Mapping[str, Any], ctx: SellPresetContext) -> SellPresetDecision:
    if ctx.in_topk is False:
        return SellPresetDecision(
            True,
            "bucket_preset:rebalance_driven:dropped_from_topk",
            ctx.reserved_for_limit_up,
        )
    rank_score = ctx.rank_score
    rank_threshold = ctx.rank_threshold
    if rank_threshold is None:
        rank_threshold = int(params.get("rank_threshold", 0) or 0) or None
    if rank_score is not None and rank_threshold is not None and rank_score > float(rank_threshold):
        return SellPresetDecision(
            True,
            "bucket_preset:rebalance_driven:rank_above_threshold",
            ctx.reserved_for_limit_up,
        )
    return SellPresetDecision(False, None, ctx.reserved_for_limit_up)


def _eval_rotation_driven(params: Mapping[str, Any], ctx: SellPresetContext) -> SellPresetDecision:
    expected = str(params.get("rotation_signal", "rotate_out") or "rotate_out").strip().lower()
    signal = str(ctx.rotation_signal or "").strip().lower()
    if signal and signal == expected:
        return SellPresetDecision(
            True,
            "bucket_preset:rotation_driven:rotate_out",
            ctx.reserved_for_limit_up,
        )
    return SellPresetDecision(False, None, ctx.reserved_for_limit_up)
...
def _register_builtin_presets() -> None:
    with _PRESET_REGISTRY_LOCK:
        ...
        register_sell_preset("rebalance_driven", _eval_rebalance_driven)
        register_sell_preset("rotation_driven", _eval_rotation_driven)
