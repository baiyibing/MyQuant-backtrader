...
def _load_and_filter(code: str, tdays, date_str: str, min_abs_resist, min_bb, positive_only,
    ...
    as_of_t = pd.Timestamp(cast(Any, today_end)).normalize()
    arr_t = adapt_columns(cast(Any, today_win), stock_code=code, as_of_date=as_of_t)
    ...
    as_of_y = pd.Timestamp(cast(Any, yest_win.index[-1])).normalize()
    arr_y = adapt_columns(cast(Any, yest_win), stock_code=code, as_of_date=as_of_y)
    ...
    bb = bb_position(cast(Any, today_win["close"].values))
...
def main():
    ...
    now = shanghai_now()
    since = now.replace(year=now.year - 15).strftime("%Y%m%d")
    until = now.replace(year=now.year + 5).strftime("%Y%m%d")
    cal_df = get_trade_days_sse(since=since, until=until)
    if cal_df is None or cal_df.empty:
        tdays = pd.DatetimeIndex([])
    else:
        tdays = pd.to_datetime(cal_df["cal_date"].astype(str), format="%Y%m%d")
