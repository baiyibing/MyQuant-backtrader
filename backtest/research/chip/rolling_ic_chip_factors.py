...
用法：
    python backtest/rolling_ic_chip_factors.py
    python backtest/rolling_ic_chip_factors.py --stocks 200 --step 20
...
def _load_data(reader, code: str) -> Optional[pd.DataFrame]:
    """加载标的全量日线数据。"""
...
def main():
    ...
    for f in factors:
        for h in horizons:
            ...
            for date, grp in sub.groupby("date"):
                ...
                ic_list.append(float(cast(Any, ic)))
    ...
    for date, grp in df.groupby("date"):
        ...
        ic_ts.append((date, float(cast(Any, ic)), len(valid)))
