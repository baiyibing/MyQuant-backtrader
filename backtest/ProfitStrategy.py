...
class Strategy8(ProfitStrategy):
    """
    策略8：金榕元交易回测。20% 止损 + 绝对涨幅分档回撤（6%~15%→+2%）。

    止损：市价较买入价回撤 >= 20 个点（默认 20%），盘中触发即按该止损卖。

    止盈（峰值涨幅 = 持仓最高价/买入价 - 1；峰值从 T+1 起算）：
      6% <= 涨幅 <= 15%（须先摸到 +2% 的两倍）→ 回撤到买入价 × 1.02
      峰值未到 +6% 不止盈。
      15% < 涨幅 <= 40% → 回撤到 +15%
      40% < 涨幅 <= 60% → 回撤到 +30%
      60% < 涨幅 <= 80% → 回撤到 +50%
      80% < 涨幅 <= 100% → 回撤到 +70%
      100% < 涨幅 <= 120% → 回撤到 +90%
      涨幅 > 120%：回撤到最高价 × 80%（不再走分档地板）。
      触发价 < 成本不止盈。

    买侧契约（由 RollingInvestmentStrategy / CSV 引擎执行）：
      尾盘涨停当日不买；T+1 09:45 市价>当日开盘则追买，否则弃买。
      CSV 引擎：已持仓票再次入池继续买，各笔 lot 独立算成本/峰值/止损止盈。
      Cerebro / Rolling 仍按标的一条持仓（均价），不做多 lot。
    """

    def __init__(self, stop_loss_pct: float = 0.20):
...
class StrategyFactory:
    ...
    PRESETS = {
        ...
        "version8": (Strategy8, {"stop_loss_pct": 0.20}),
    }
