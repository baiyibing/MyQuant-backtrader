...
# StockCodeProcessor: transitional import from legacy backtest module
try:
    from backtest.qmt_utils_new import StockCodeProcessor  # type: ignore[reportUnusedImport]
except ImportError:
    StockCodeProcessor = None  # fallback to local float_shares.parquet
...
FLOAT_SHARES_PATH = os.path.join(BASE_DIR, "float_shares.parquet")
...
def _get_all_a_stock_codes() -> List[str]:
    ...
    if os.path.exists(FLOAT_SHARES_PATH):
        ...
        fallback_source = "float_shares.parquet"
    else:
        front_dir = os.path.join(BASE_DIR, "period=1d", "dividend_type=front")
...
def _get_codes_for_minute_backfill() -> List[str]:
    ...
    daily_dir = os.path.join(BASE_DIR, "period=1d", "dividend_type=none")
    minute_dir = os.path.join(BASE_DIR, "period=1m", "dividend_type=none")
    ...
    for code in sample:
        ...
        try:
            ...
            for day in recent_days:
                ...
                has_close = "1500" in day_bars["minute"].values
                if not has_close:
                    stale.add(code)
                    break
        except Exception:
...
def _rebuild_duckdb(rebuild_period: str) -> None:
    ...
    for period, adjust in periods_to_build:
        try:
            ...
            logger.info("%s rebuilt: %.0f MB in %.0fs", db_path.name, size_mb, elapsed)
        except Exception as e:
            logger.warning("%s .duckdb rebuild failed (non-fatal): %s", period, e)
...
def _gate_time_window(period: str) -> None:
    ...
    使用周一至周五近似判断交易日（本 fork 已移除 oskh_core 交易日历），
    ...
    # Standalone BT fork: weekday proxy (Mon-Fri). Holidays are treated as trading days.
...
def _gate_check_parquet_health(period: str = '1m') -> int:
    ...
    parquet_glob = os.path.join(
        BASE_DIR, f'period={period}', 'dividend_type=none', 'symbol=*', 'data.parquet'
    ).replace('\\', '/')
...
def _gate_verify_freshness(period: str, done_count: int) -> None:
    ...
    parquet_dir = _Path(BASE_DIR) / f'period={period}' / 'dividend_type=none'
    ...
    if benchmark.exists():
        try:
            ...
            if not df.empty and hasattr(df.index, 'max'):
                latest = df.index.max()
                days_behind = (shanghai_now().replace(tzinfo=None) - pd.Timestamp(latest)).days
                if days_behind > 3:
                    logger.warning(
...
def _gate_inventory_snapshot(period: str = '1m', adjust_type: str = 'none') -> None:
    ...
    parquet_glob = os.path.join(
        BASE_DIR, f'period={period}', f'dividend_type={adj}', 'symbol=*', 'data.parquet'
    ).replace('\\', '/')
    ...
    for adjust in adjust_types:
        ...
        for idx, batch in enumerate(batches, start=1):
            ...
            try:
                ...
                if result and isinstance(result, dict) and not result.get("_download_error"):
