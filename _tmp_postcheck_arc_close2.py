"""Post-run checks for fill-copy plain parity run (stacked tip: marks-hotset + fill-copy)."""
import hashlib
import json
...
new = phase / "out-v2-pack-spans-arc-close2-95431fc" / "joint-return-control-only-50-5-narrow-clock-patch-614"
prev = phase / "out-v2-pack-spans-marks-hotset" / "joint-return-control-only-50-5-narrow-clock-patch-614"
targets = {
...
print("=== SHA256 new vs target vs marks-hotset(#189) file ===")
ok_parity = True
...
snap = json.loads((phase / "_tmp_forbidden_mtime_snapshot_arc_close2.json").read_text(encoding="utf-8"))
changed, missing = [], []
...
knives_ok = "backfill_scan_steps" in text and "cannot rewrite history" in text
print("\n=== business file (both-knife tree) ===")
print("stock_ok:", stock in text, "| pin_gone:", pin not in text, "| knives_ok:", knives_ok)
